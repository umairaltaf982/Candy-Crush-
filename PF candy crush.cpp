...
int move_format_validity_hard(char move[5])
{
	if (move[0] == 'S' || move[0] == 's') { return 9; }
	if (move[0] < '0' || move[0] >= '8') { return 1; }
	if (move[1] < '0' || move[1] >= '8') { return 1; }
...
dis:	cout << "1.playgame\n2.credits\n3.instructions\n4.score\n5.exit\nEnter your choice : ";
	...
	else if (display == 4)
	{
		string loaddata[6];
		ifstream load;
		load.open("name score.txt", ios::app);
		system("CLS");
		if (!load.eof()) {
			...
			//convert string to integers so that score can be compared
			int s1 = atoi(loaddata[1].c_str()), s3 = atoi(loaddata[3].c_str()), s5 = atoi(loaddata[5].c_str());
			//compare and output score in accending order
			if (s1 >= s3 && s1 >= s5)
			{
		...
		{
		nogamesave:				cout << "you have not played any game!!\n";
		}
	...
	A:	cout << "\n\tGame mode\nEasy made\tHard mode\n\nEnter your choice: ";
		...
		shuffle:	for (int i = 0; i < 8; i++)// this loop is to generate candies on board 
		{
			for (int j = 0; j < 8; j++)
			{
		...
		do {
			...
			cout << "Time remaining : " << time / 1000 << " sec" << endl;
		...
		ofstream save;
		save.open("name score.txt", ios::app);
		save << name << endl;
		save << score << endl;
